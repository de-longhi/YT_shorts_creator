...
def main():
    ...
    audio_paths = []
    for fact in facts:
        text_to_speech.synthesize_speech(fact, "src/text_to_speech/out/voiceover{i}")
        audio_paths.append(f"src/text_to_speech/out/voiceover{i}")
        i += 1

    print("Creating video...")
    youtuber = video_editor.Youtuber(
        "assets/background_videos/Parkour.mp4", audio_paths, facts, "./out/"
    )
